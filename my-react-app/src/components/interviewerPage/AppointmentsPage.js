...
const AppointmentsPage = () => {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        ...
        setError(err.response ? err.response.data.detail : 'Failed to fetch appointments');
      }
    };
  ...
  const handleFilter = () => {
    ...
    const matchesStatus = (appointment) =>
      !statusFilter || (statusFilter === 'done' ? appointment.done : !appointment.done);
    ...
    return appointments.filter((appointment) =>
      matchesSearch(appointment) && matchesStatus(appointment) && matchesDuration(appointment)
    );
  };
