...
const SkillsPage = () => {
  ...
  const { data: skillsData, isLoading, isError, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const processedSkills = skills.reduce((acc, skill) => {
        const existingSkill = acc.find(
          (item) =>
            item.title === skill.skill && item.category === skill.category
        );
  ...
  const handleMouseEnter = (skill) => {
    ...
    if (skill.both_practical_theoretical) {
      if (!skill.types.includes("theoretical")) {
        ...
        message += "You have taken both theoretical and practical tests.";
        setHoveredSkill(skill);
      }
    }
  ...
  {group.skills.map((skill, i) => (
    ...
    >
      ...
      {(hoveredSkill === skill || hoveredCategory === group.category) && (
        <div
          className="absolute -top-28 left-1/2 transform -translate-x-1/2 bg-brand-dark-blue text-white text-sm px-3 py-2 rounded-lg shadow-md z-10  w-48 h-20 transition-opacity duration-300 opacity-100 pointer-events-none"
        >
          {message}
          {/* Triangle Indicator */}
          <div className="absolute bottom-[-6px] left-1/2 transform -translate-x-1/2 w-3 h-3 bg-brand-dark-blue rotate-45 z-10" />
        </div>
      )}
      ...
      <button
        type="button"
        className="relative bg-brand-blue text-white px-4 py-2 rounded-lg hover:bg-brand-dark-blue transition-all duration-300"
        style={{
          width: `${Math.max(skill.title.length * 10, 150)}px`,
        }}
      >
        <span className="flex items-center justify-center">
          {skill.title}
          {skill.verified && <span className="ml-2 text-green-500">&#10003;</span>}
        </span>
        {/* Progress Bar */}
