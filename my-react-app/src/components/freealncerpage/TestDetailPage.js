...
const TestDetailPage = () => {
  ...
  const { useFetchSkillTest } = useContext(SkillTestContext);
  const { data: testDetails, isLoading, error } = useFetchSkillTest(id, type);

  const handleTestStart = () => {
    if (type === "theoretical") {
      const navigatePath =
        startingPath === "/skill-test"
          ? `/theory-skill-test/${id}`
          : `/theory-test/${id}`;
    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-8 bg-gray-100">
      <div className="w-full max-w-3xl bg-white shadow-lg rounded-lg overflow-hidden">
        <div className="p-6 bg-gradient-to-r from-brand-blue to-brand-dark-blue text-white">
          {type === "theoretical" ? (
            <h2 className="text-3xl font-normal">{testDetails?.title} Theoretical Test</h2>
          ) : (
            <h2 className="text-3xl font-bold">{testDetails?.skill_type} Practical Test</h2>
          )}
        ...
        <div className="p-6">
          ...
          <ul className="list-disc list-inside space-y-2">
            {testDetails?.duration_in_minutes && (
              <li>Duration: {testDetails?.duration_in_minutes} minutes</li>
            )}
          ...
          <div className="mt-12 flex justify-center">
            <button
              ...
              className="bg-brand-green w-[30%] text-white font-semibold py-3 px-8 rounded-lg hover:bg-brand-dark-green transition-colors duration-300"
            >
              Start
            </button>
          </div>
