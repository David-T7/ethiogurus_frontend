...
const EditDisputeResponsePage = () => {
    ...
    const fetchDisputeData = async () => {
      try {
        ...
        setResponse({
          ...
          decision: '',
          documents: [],
          supporting_documents: disputeResponse.data.supporting_documents || []
        });
        setInitialResponse(disputeResponse.data);
  ...
  const handleSubmit = async (e) => {
    ...
    for (const file of response.documents) {
      ...
      documentFormData.append('uploaded_by', freelancerId);
    ...
    try {
      if (response.decision === 'accept') {
        await axios.patch(`http://127.0.0.1:8000/api/disputes/${response.dispute}/`, {
         ...
         await axios.patch(`http://127.0.0.1:8000/api/dispute-response/${disputeResponseId}/`, {
          response: 'accepted',
        }, {
          headers: { Authorization: `Bearer ${token}` },
        });
       ...
       await axios.patch(`http://127.0.0.1:8000/api/disputes/${response.dispute}/`, {
        ...
        // Update the dispute response with the uploaded document IDs
        console.log("form data is ",payload)
        const disputeResponse = await axios.patch(`http://127.0.0.1:8000/api/dispute-response/${disputeResponseId}/`, {
          ...payload,
  ...
  <div className="mb-6 p-4 border rounded-lg border-gray-300 bg-gray-50">
    ...
    <label htmlFor="forwardToResolutionCenter" className="text-gray-700">Check to forward the dispute to the resolution center</label>
      ...
      <div className="mb-6">
        ...
        <input id="title" name="title" value={response.title} onChange={handleInputChange} placeholder="Enter a title for your response" className="w-full border p-2 rounded-lg" />
      </div>
      <div className="mb-6">
        <label htmlFor="description" className="block text-lg font-normal text-brand-blue mb-2">Description</label>
        <textarea id="description" name="description" value={response.description} onChange={handleInputChange} rows="4" className="w-full border p-2 rounded-lg" placeholder="Enter your comments..."></textarea>
      </div>
